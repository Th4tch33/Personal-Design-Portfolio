...
const numIconsFarBack = 20;
const numOfIcons = numIconsFront + numIconsMid + numIconsBack + numIconsFarBack;
...
class iconBuilder {
    ...
    constructor(x: number, y: number, direction: boolean, vY: number, r: number , vR: number, s: number) {
        this.x = x;
        this.y = y;
...
window.onload = function()
{
    ...
    for(let i = 0; i < numOfIcons; i++) {
        ...
        if (i < numIconsFront){
            ...
            icons[i] = new iconBuilder(getRandomIntRange(windowWidth + iconXSpawnOverflow, -iconXSpawnOverflow), getRandomIntRange(iconFloor, iconCeiling), getRandomBool(), getRandomIntRange(initialFallMax, initialFallMin) * iconMidSize, getRandomInt(360), getRandomInt(rotationSpeed), iconMidSize);
